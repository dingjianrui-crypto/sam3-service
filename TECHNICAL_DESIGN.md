...
The visual renderer draws the upper-body chain for every discipline. Canoe additionally draws hip-to-knee and knee-to-ankle segments and a knee-centered angle arc when the required landmarks are confident. Skeleton strokes remain thin, and measured elbow, shoulder, and knee joint markers use the corresponding angle-label colors. Playback keeps five fixed slots—L Elbow, R Elbow, Torso, L Shoulder, and R Shoulder—at a 10% top offset, using `--` for unavailable measurements. Canoe jobs append L Knee and R Knee using the existing three-point knee angles; Kayak jobs retain the five-slot row. Export uses `event_metric_center_offset_percent` to position the same discipline-specific row. The former cumulative Event metrics table is not rendered, while catch/exit event geometry, labels, and freeze behavior remain independent and unchanged.

Ordinary exported paddle-angle numbers are rendered only in the configured top or bottom degree block and do not draw numeric labels or leaders near paddle centerlines. On a catch/exit frame, the exporter replaces the matching degree-block slot with the backdated event-time angle and event color, and also prints the number immediately around the colored event arc, before duplicating that fully annotated image for any configured freeze. With all event paddles selected, simultaneous events are assigned to the nearest detected paddle slots.
