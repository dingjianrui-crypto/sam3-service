...
Canoe analysis uses a separate pendulum pipeline. Raw SAM paddle tracks are candidates only: the exporter anchors paddles as stable slots on their associated boat by projecting their centers onto the detected travel-oriented boat axis. A raw track-ID change inside one boat slot does not create a new paddle phase. The pipeline canonicalizes the waterline screen-left to screen-right and calculates each moving paddle axis as an undirected `[0°, 180°)` angle. Angles in `0°-80°` vote right travel; angles in `100°-180°` vote left travel; the `80°-100°` vertical band casts no vote. Static tracks with less than `8°` axis span are ignored. Direction requires at least five eligible observations and 70 percent weighted consensus. After direction locks, a canoe phase is a `minimum angle -> peak angle -> next minimum angle` sequence. The confirmed minimum closes the previous phase and is reused as the next phase start. Phase angle uses a robust slot-level boat reference direction, so occasional per-frame waterline angle jitter does not create artificial phase boundaries. Within each phase, catch restores rising-frame paddle length from the phase-start minimum-angle length until `90°`; a shortened frame can inherit only when the required stretch is within `15%`. Catch candidates are the before/after frames around a restored centerline-waterline crossing, and the chosen catch is the one whose active endpoint is closest to the local event-frame waterline. Exit is the phase peak frame after the restore decrease confirms that peak. If a clip starts mid-restore and the phase peak is also the first phase sample, that first peak is accepted as exit only when its active endpoint is close to the local event-frame waterline; later peaks do not use this proximity gate. Rendered event geometry still uses the local event-frame reference line. Canoe event angles use a signed `[-180°, 180°)` blade-vector angle in a direction-normalized waterline basis. These thresholds are provisional and require representative real-video calibration. Unknown direction emits no canoe events and never falls back to kayak rotation logic.
