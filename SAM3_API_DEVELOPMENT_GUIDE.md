...
| `event_paddle_index` | integer, `1` to `4` | none (`ALL`) | Restrict catch and exit analysis to the indexed physical paddle, ordered front-to-back in the detected travel direction; must not exceed `metric_count` when both are supplied |
...
`event_paddle_index` affects only catch and exit analysis. Event paddles are
tracked first and ranked once per reference boat by their median center
projection along the direction-normalized boat axis. Index `1` is the
frontmost paddle in the moving direction, and the assignment remains attached
to that physical track for the complete export. Omitting the parameter analyzes
all eligible paddles. If the requested index is unavailable, no paddle from
that reference boat is substituted and no event is emitted for it. Angle and
SPM rendering continue to use `metric_count` independently.
...
Catch and exit use the active endpoint's signed waterline depth and a finite-width tolerance band. A catch candidate begins on the first observed above-to-water transition while moving downward. An exit candidate begins on the first observed underwater-to-air transition while moving upward. The transition must occur on the finite detected waterline. Two consecutive compatible samples confirm the transition, while the event timestamp, angle, and geometry are backdated to its first sample. Candidates expire after 400 ms, and a detection gap longer than that reinitializes phase/contact observation rather than synthesizing a transition across missing frames.
