...
MediaPipe retains left/right wrists, elbows, shoulders, hips, knees, and ankles for one primary athlete. The API continues to expose elbow, shoulder, hip, and knee values as ordinary three-point 2D angles for backward compatibility. The UI and exporter render only wrists, elbows, shoulders, hips, and their connecting upper-body segments; knees, hip-to-knee links, hip angles, and knee angles are not displayed. Every available left/right elbow and shoulder angle uses a joint-centered arc without a leader line. Five distinctly colored fixed positions show L Elbow, R Elbow, Torso, L Shoulder, and R Shoulder; an unavailable measurement displays `--`. Torso is the signed shoulder-midpoint-to-hip-midpoint lean angle against the video vertical axis. Its sign uses the canonicalized configured waterline or centerline direction, with centerline fallback when a requested waterline is unavailable.
...
| `include_body_motion` | boolean | `false` | Draw the upper-body skeleton, bilateral elbow/shoulder angles, and signed torso lean; returns `BODY_MOTION_UNAVAILABLE` when completed pose results are absent |
