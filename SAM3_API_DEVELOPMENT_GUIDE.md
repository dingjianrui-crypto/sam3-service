...
The finite-width paddle centerline is divided into two blade-end contact zones. A catch candidate begins when a dry blade zone first overlaps the finite-width boat centerline or waterline while moving toward it. An exit candidate begins when the same immersed blade loses overlap while moving away. Two consecutive samples confirm either transition, and the event timestamp, angle, and geometry are backdated to the first transition sample. When first contact occurs while the angle state machine is still confirming the fourth phase, the confirmed contact is retained for up to 1.5 seconds and emitted at its original frame as soon as that phase gate opens.
