...
MediaPipe retains left/right wrists, elbows, shoulders, hips, knees, and ankles for one primary athlete. The API exposes elbow, shoulder, hip, and knee values as ordinary three-point 2D angles. Kayak overlays render the upper-body chain. Canoe overlays additionally render hip-to-knee and knee-to-ankle segments plus a knee-centered angle arc whenever all required landmarks are confident. Measured elbow, shoulder, and knee joints use the same colors as their angle labels. Five fixed positions show L Elbow, R Elbow, Torso, L Shoulder, and R Shoulder; Canoe jobs append L Knee and R Knee from the existing knee angles. Kayak jobs retain the five positions, and any unavailable measurement displays `--`. Torso is the signed shoulder-midpoint-to-hip-midpoint lean angle against the video vertical axis. Its sign uses the canonicalized configured waterline or centerline direction, with centerline fallback when a requested waterline is unavailable.
...
| `include_event_metrics` | boolean | `false` | Deprecated compatibility parameter; accepted but no longer renders a cumulative event table |
| `include_body_motion` | boolean | `false` | Draw the selected body skeleton, bilateral elbow/shoulder angles, and signed torso lean; Canoe jobs also show bilateral knee angles. Returns `BODY_MOTION_UNAVAILABLE` when completed pose results are absent |
| `body_joint_names` | comma-separated string | all 12 body joints | Joints to draw when body motion is included: `left_` or `right_` wrist, elbow, shoulder, hip, knee, and ankle. An empty value draws no skeleton joints or lines. Unsupported names return `INVALID_BODY_JOINT_NAMES`. |
| `include_boat_line` | boolean | `true` | Draw the selected boat centerline or waterline. Hiding it does not remove the internal boat reference used for angles, SPM, or catch/exit analysis. |
| `event_metric_center_offset_percent` | number, `-45` to `45` | `5.5` | Body-motion metric-row vertical inset: nonnegative from the top, negative from the bottom |
...
The web viewer separates Playback controls from Export controls. Playback keeps global overlay opacity, boxes and IDs, optional angle preview, and one visibility/overlay-target row per prompt so different object types can independently show their full mask, centerline, or generated waterline. Export owns `include_angles`, reference and target prompts, boat-line visibility, degree placement, font size, metric count, center offset, SPM, catch and exit events, event-paddle selection, event freeze time, and MP4 generation. The angle target row shows the paddle checkbox followed by a Boat checkbox; Boat controls whether the selected centerline or waterline is burned into the exported video without changing calculations. The angle switch, degree offset, and font size are grouped with the angle controls, while the SPM switch is grouped with the general export controls. The signed degree offset measures positive percentages down from the top and negative percentages up from the bottom; it defaults to `5.5%` for landscape video and `16%` for portrait video. The export reference and target selections also drive the optional live angle preview.

When catch or exit is enabled, export selects event analysis from the job's persisted `paddling_discipline`. Kayak analysis performs the established two-pass rotation pipeline: it merges nearby collinear paddle fragments, associates them with persistent physical-paddle tracks across raw track-ID changes, stabilizes line length and endpoint ordering, and estimates rotation direction from the undirected paddle-axis sequence. Clockwise rotation maps to rightward kayak travel; anticlockwise rotation maps to leftward travel. Direction must have at least five valid angular deltas, 45 degrees of accumulated motion, and 75 percent directional consensus. Low-confidence direction segments emit no events.

Canoe analysis uses a separate pendulum pipeline. Raw SAM paddle tracks are candidates only: the exporter anchors paddles as stable slots on their associated boat by projecting their centers onto the detected travel-oriented boat axis. A raw track-ID change inside one boat slot does not create a new paddle phase. Travel direction primarily comes from completed body-motion landmarks. For each valid pose frame, vertical screen axes pass through the left and right hips: both shoulders to the right of their corresponding hip axes vote right travel, both to the left vote left, and mixed-side frames cast no vote. At least five eligible frames and a simple majority are required. If pose direction is unavailable, the existing paddle-axis fallback canonicalizes the waterline screen-left to screen-right: `0°-80°` votes right, `100°-180°` votes left, and the `80°-100°` band casts no vote; fallback direction retains the 70 percent weighted-consensus threshold. After direction locks, a canoe phase is a `minimum angle -> peak angle -> next minimum angle` sequence. The confirmed minimum closes the previous phase and is reused as the next phase start. Phase angle uses a robust slot-level boat reference direction, so occasional per-frame waterline angle jitter does not create artificial phase boundaries. Within each phase, catch restores rising-frame paddle length from the phase-start minimum-angle length until `90°`; shortened frames inherit the previous length, while length growth updates the inherited value only when the growth is within `15%`; larger jumps inherit the previous length as well. Catch candidates are the before/after frames around a restored centerline-waterline crossing, and the chosen catch is the one whose active endpoint is closest to the local event-frame waterline. Exit is the phase peak frame after the restore decrease confirms that peak. If a clip starts mid-restore and the phase peak is also the first phase sample, that first peak is accepted as exit only when its active endpoint is close to the local event-frame waterline; later peaks do not use this proximity gate. Rendered event geometry still uses the local event-frame reference line. Canoe event angles use a signed `[-180°, 180°)` blade-vector angle in a direction-normalized waterline basis. These thresholds are provisional and require representative real-video calibration. Unknown direction emits no canoe events and never falls back to kayak rotation logic.
...
Catch/exit event geometry is drawn on the event frame independently of the deprecated `include_event_metrics` parameter: the paddle centerline is cyan, its matched boat centerline or waterline is amber, and the directed angle arc is red for catch or green for exit. The numeric event angle is placed both in the configured top or bottom degree block and immediately around the event arc, and the same completed frame is used throughout an optional freeze. When all event paddles are selected, simultaneous events replace their nearest detected paddle slots. The former cumulative catch/exit event table is no longer rendered.

When body motion is included in export, the web client exposes a joint checklist with all left and right wrists, elbows, shoulders, hips, knees, and ankles selected by default. Selected adjacent joints on each side are linked automatically in wrist-elbow-shoulder-hip-knee-ankle order. The shoulders and hips are also linked across the body when both matching joints are selected. Left and right joints use distinct colors, while all skeleton links use one shared cyan color. Joint selection controls skeleton dots, links, and angle arcs; the existing body metric row remains present for compatibility. `event_metric_center_offset_percent` controls the fixed L Elbow, R Elbow, Torso, L Shoulder, and R Shoulder row. A nonnegative value anchors its top edge that percentage of the video height below the top (`+10` means 10% down). A negative value anchors its bottom edge by the absolute percentage above the bottom (`-10` means 10% up). Zero means no top inset. The existing `metric_center_offset_percent` remains an independent compatibility setting for the live paddle-degree and SPM rows.
