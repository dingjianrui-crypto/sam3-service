...
When catch or exit is enabled, export performs an event-analysis pass before rendering. It measures each target paddle endpoint on the water-facing side of the nearest boat reference line, normalizes depth by reference-line length, and applies separate entry and release thresholds. Two consecutive samples confirm a transition, while the event timestamp and geometry are assigned to the first crossing sample. Nearby same-type events within 250 ms are merged to suppress duplicates from briefly split paddle detections.

At each enabled event, the exporter freezes the complete source frame for `event_hold_seconds`; later source frames resume after the inserted pause. Events from different paddles within 250 ms share one freeze so a synchronized crew does not create several consecutive pauses. SPM and selection calculations remain on the original source timeline. The frozen frame draws the matched paddle and boat reference rays, an angle arc, and the numeric event angle. Catch annotations are green and exit annotations are red.
