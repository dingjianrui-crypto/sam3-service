...
The web viewer separates Playback controls from Export controls. Playback keeps global overlay opacity, boxes and IDs, optional angle preview, and one visibility/overlay-target row per prompt so different object types can independently show their full mask, centerline, or generated waterline. Export owns `include_angles`, reference and target prompts, degree placement, font size, metric count, center offset, SPM, catch and exit events, optional event paddle length, event freeze time, and MP4 generation. The export reference and target selections also drive the optional live angle preview.
