...
The video toolbar provides Rectangle and Undo tools. Rectangle pauses playback and lets the user drag an export area directly over the video viewport. A dedicated timeline below the viewport shows each saved rectangle position as a numbered milestone with a vertical tick. Milestones change color as playback passes them, and the time readout reports the total keyframe count. Seeking to a new time and moving or redrawing the rectangle records the keyframe at the timeline slider's exact current timestamp; the client and export renderer linearly interpolate its position and size between keyframes, then hold the first and last rectangles outside the keyed interval. Undo removes the most recently edited keyframe and returns export filtering to the full frame after the last keyframe is removed.
