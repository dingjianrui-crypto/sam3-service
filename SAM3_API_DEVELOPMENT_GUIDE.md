...
| `include_catch` | boolean | `false` | Detect catch events and draw `CATCH` near the associated paddle |
| `include_exit` | boolean | `false` | Detect exit events and draw `EXIT` near the associated paddle |
...
The web viewer separates Playback controls from Export controls. Playback keeps global overlay opacity, boxes and IDs, optional angle preview, and one visibility/overlay-target row per prompt so different object types can independently show their full mask, centerline, or generated waterline. Export owns `include_angles`, reference and target prompts, degree placement, font size, metric count, center offset, SPM, catch and exit events, event freeze time, and MP4 generation. The export reference and target selections also drive the optional live angle preview.

When catch or exit is enabled, export performs a phase-aware event-analysis pass before rendering. Collinear, nearby paddle records are first merged into one physical paddle observation, then associated with persistent per-boat paddle state across track-ID changes and short detection gaps. The exporter stabilizes orientation and recent line length so partial blade masks do not abruptly move the reconstructed paddle endpoint.

The finite-width paddle centerline is divided into two blade-end contact zones. A catch candidate begins when a dry blade zone first overlaps the finite-width boat centerline or waterline while moving toward it. An exit candidate begins when the same immersed blade loses overlap while moving away. Two consecutive samples confirm either transition, and the event timestamp, angle, and geometry are backdated to the first transition sample.

The acute paddle-to-boat angle is smoothed and treated as four monotonic phases per complete paddle revolution: `0° → 90°`, `90° → 0°`, `0° → 90°`, and `90° → 0°`. A phase advances only after a sustained direction reversal whose preceding movement reached the corresponding low or high angle region. Catch and exit maintain independent phase clocks, and the next event of the same type cannot be confirmed until four phases have elapsed. There is no two-phase opposite-blade exception.

Catch eligibility does not depend on a previously confirmed exit, and exit eligibility does not depend on a previously confirmed catch. Consequently, one missed transition cannot leave a stale immersed/dry flag that blocks later cycles. Pending two-sample candidates still expire after 400 ms, while phase progression and physical-paddle identity continue across short detection gaps.

Nearby collinear same-type events within 250 ms are merged after state analysis as an additional safeguard. This produces at most one event for a briefly fragmented physical paddle while retaining spatially separate paddles in a multi-athlete boat.

At each enabled event, the exporter inserts a pause at the event timestamp and freezes the complete source frame for `event_hold_seconds`. Audio is silent during the pause, then video and audio resume from the same source timestamp without skipping source content. Events from different paddles within 250 ms share one freeze so a synchronized crew does not create several consecutive pauses. SPM and selection calculations remain on the original source timeline. Only the inserted pause frames redraw the paddle centerline in cyan and its matched boat centerline or waterline in amber, with a white angle arc and numeric degree. No catch or exit text is rendered. The temporary highlights disappear and the configured overlay colors return when normal playback resumes.
