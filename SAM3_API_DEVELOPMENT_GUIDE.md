...
MediaPipe retains left/right wrists, elbows, shoulders, hips, knees, and ankles for one primary athlete. The API exposes elbow, shoulder, hip, and knee values as ordinary three-point 2D angles. The UI and exporter render wrists, elbows, shoulders, hips, and their connecting upper-body segments; knees and hip-to-knee links are not drawn. Every available left/right elbow and shoulder angle uses a joint-centered arc without a leader line. Five distinctly colored fixed positions show L Elbow, R Elbow, Torso, L Shoulder, and R Shoulder; Canoe jobs append L Knee and R Knee from the existing knee angles. Kayak jobs retain the five positions, and any unavailable measurement displays `--`. Torso is the signed shoulder-midpoint-to-hip-midpoint lean angle against the video vertical axis. Its sign uses the canonicalized configured waterline or centerline direction, with centerline fallback when a requested waterline is unavailable.
