...
Before event detection, each directed `0°-180°` stroke restores paddle length with independent forward `0°-90°` and reverse `180°-90°` envelopes. The mask-only completeness CNN is the authoritative candidate gate: a mask predicted complete can seed or increase the applicable envelope directly, while cropped or unknown masks may only inherit a previously verified length. No large-change threshold, appearance-based rejection, or temporal-tolerance rejection is applied to a CNN-complete length. Inherited geometry anchors the inactive endpoint and extends only the active endpoint, and accepted lengths remain isolated to their phase, cycle, and continuous track segment. Frames without a verified event length remain ineligible to start or confirm catch/exit events, but their raw centerlines continue endpoint-orientation and directed-phase tracking. Only a genuine paddle-observation gap resets that physical continuity state.
